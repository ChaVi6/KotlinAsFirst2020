...
fun markdownToHtmlLists(inputName: String, outputName: String) {
    val lines = File(inputName).readText().trim().split("\n")
    val result = mutableListOf<String>()
    val elementsToClose = mutableListOf<String>()
    var listBefore = 'I'
    var tabsBefore = -1
    val mapDotToTag = mapOf(
        'O' to "ol",
        '*' to "ul"
    )

    fun writeTextToResult(text: String) {
        result.add("<li>")
        result.add(text)
        result.add("</li>")
    }
    ...
    for (i in lines.indices) {
        val line = lines[i]
        var dot = line.trim()[0]
        val tabs = line.indexOf(dot) / 4
        val text = line.substring(tabs * 4).split(" ").drop(1).joinToString(separator = " ")
        ...
        } else if (tabs < tabsBefore) {
            result.add("</${mapDotToTag[listBefore]}>")
            result.add("</li>")
            elementsToClose.remove("li")
            elementsToClose.remove("${mapDotToTag[listBefore]}")
            writeTextToResult(text)
        } else if (listBefore != dot) {
            result.add("</${mapDotToTag[listBefore]}>")
            elementsToClose.remove("${mapDotToTag[listBefore]}]}")
            writeTextToResult(text)
        } else {
    ...
    File(outputName).writeText("<html><body><p>" + result.joinToString(separator = " ") + "</p></body></html>")
}
